...
if __name__ == '__main__':
    ...
    neg_idx = np.delete(np.array([i for i in range(len(test_dataset))]), pos_idx)
    ...
    benign_namelist = list(set(list(range(args['num_users']))) - set(args['adversary_list']))
    ...
    for epoch in tqdm(range(args['epochs'])):
        ...
        benign_num = m - len(adversarial_name_keys)  # benign agent for each epoch, 6
        random_agent_name_keys = random.sample(benign_namelist + nonattacker, benign_num)
            ...
            if args['is_poison'] and idx in args['adversary_list']:
                for temp_index in range(0, len(args['adversary_list'])): # 0-3
                    if int(idx) == args['adversary_list'][temp_index]:
                        ...
                        local_poison_epochs =args[str(temp_index) + '_poison_epochs']
                        break
        ...
        global_weights = average_weights(local_weights)#, local_poisoned, alpha=1)
